...
trait HasFile
{
    public function upload_file(Request $request, string $column, string $folder)
    {
        return $request->hasFile($column) ? $request->file($column)->store($folder) : null;
    }

    public function update_file(Request $request, Model $model, string $column, string $folder)
    {
